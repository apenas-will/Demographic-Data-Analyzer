...
def calculate_demographic_data(print_data=True):
    ...
    num_min_workers = df[(df['hours-per-week'] == df['hours-per-week'].min()) & (df['salary'] == ">50k")] 

    rich_percentage = (df[(df['hours-per-week'] == df['hours-per-week'].min()) & (df['salary'] == '>50K')]['hours-per-week'].count() / df[df['hours-per-week'] == df['hours-per-week'].min()]['hours-per-week'].count()) * 100

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = df[df['salary'] == '>50K']['native-country'].value_counts().idxmax()
    highest_earning_country_percentage = round((df[df['salary'] == '>50K']['native-country'].value_counts().max() / df[df['native-country'] == 'United-States']['native-country'].count()) * 100, 1)
